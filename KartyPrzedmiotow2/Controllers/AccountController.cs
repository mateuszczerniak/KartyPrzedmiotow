...
namespace KartyPrzedmiotow2.Controllers
{
    ...
    public class AccountController : Controller
    {
                ...
                case SignInStatus.Success:
                    {
                        ...
                        var isAdmin = context.Users.Any(u => u.UserName == model.Email && u.Roles.Any(r => r.RoleId == adminRole.Id));
                        if (isAdmin && (string.IsNullOrWhiteSpace(returnUrl) || returnUrl == "/"))
                        {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new Uzytkownik { UserName = model.Email, Email = model.Email, Imie = model.Imie, Nazwisko = model.Nazwisko };
